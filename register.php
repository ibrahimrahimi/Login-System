...
<body>
    <?php 
        ...
        if(isset($_POST['submit'])){
            ...
            if(!$username || !$password || !$passconf || !$email){
                echo "<center> You need to fill all the required fields in the form!</center>";
            }else{
                if(strlen($username) > 32 || strlen($username) < 3 ){
                    echo "<center> Your <b>Username</b> should be between 3 and 32 character long!</center>";
                }else{
                    ...
                    if($num == 1){
                        echo "<center>The <b>Username</br> you have choien is already taken!</center>";
                    }else{
                        if(strlen($password) < 5 || strlen($password) > 32){
                            echo "<center> Your <b>Password</b> should be between 5 and 32 characters!</center>";
                        }else{
                            if($password != $passconf){
                                echo "<center> The <b>Password you supplied did not match the confirmation password!</center>";
                            }else{
                                $checkemail = "/^[a-z0-9]+([_\\.-][a-z0-9]+)*@([a-z0-9]+([\.-][a-z0-9]+)*)+\\.[a-z]{2,}$/i";
                                if(!preg_match($checkemail, $email)){
                                    echo "<center> The <b>Email</b> is not valid, must be name@server.ltd!</center>";
                                }else{
                                    $res1 = mysqli_query($link, "SELECT * FROM `users` WHERE `email` = '".$email."'");
                                    $num1 = mysqli_num_rows($res1);
                                    if($num1 == 1){
                                        echo "<center> The <b>Email</b> address you supplied is already taken</center>";
                                    }else{
                                        $registerTime = date('U');
                                        $online = date('U')+60;
                                        $active = 0;
                                        ...
                                        if($link->query($res2) === TRUE){
                                            ...
                                            echo "<center>You have successfully registered, please visit your inbox to activate your acount!</center>";
                                        }else {
    ...
    <form method="post" action="register.php">
        <table cellpadding="2" cellspacing="0" border="0">
            ...
            <tr>
                <td><input placeholder="Confirm Password" type="password" name="passconf"></td>
            </tr>
            <tr>
